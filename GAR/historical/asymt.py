...
K = np.vectorize(K_plain, otypes=[float], cache=False)
alpha_star = np.vectorize(alpha_star_plain, otypes=[float], cache=False)
...
def asymt_log_likelihood(sample, theta): # Theta is a coefficient vector
    ...
    lkl_vect = np.vectorize(lkl, otypes=[float], cache=False)
...
## To improve speed, vectorize the ancillary functions (used everywhere else)
D = np.vectorize(D_plain, otypes=[float], cache=False)
D_prime = np.vectorize(D_prime_plain, otypes=[float], cache=False)
